    ...
    >
      <template v-slot:activator="{ on }">
        ...
        >
          <v-img
              v-if="poll.image"
              :src="(poll.image.startsWith('/') ? 'https://vrotakus.my-server.nl':'') + poll.image"
              alt="poll"
          ...
          <v-card-text>
            <v-data-table :items="poll.options"
                          :headers="[{text:'Option', value: 'content'},{text:'Choice', value: 'voted', width: 50}]"
                          hide-default-footer>
              <template v-slot:item.content="{item}">
                {{ item.media ? item.media.title : item.content }}
              </template>
              <template v-slot:item.voted="{item}">
                <v-select v-model="item.voted" :items="[0,1,2,3,4,5]" @change="vote(item)"/>
              </template>
            </v-data-table>
          </v-card-text>
        </v-card>
...
export default class PollCard extends Vue {
  ...
  vote(option: PollOption) {
    if (this.isLoggedIn) {
      axios.post("/api/polls/vote", {option: option.id, choice: option.voted});
    } else {
