...
import {MessageReaction, RichEmbed, TextChannel, User as DiscordUser} from 'discord.js';
...
export class PollDiscord {
    ...
    public static getChannel() {
        return client.channels.get(discordConfig.channel.poll) as TextChannel;
    }

    @On('ready')
    public async init() {
        await PollDiscord.getChannel().fetchMessages();
    ...
    public async addVote(messageReaction: MessageReaction, user: DiscordUser) {
        if (user.bot) {
            return;
        }
    ...
    public async removeVote(messageReaction: MessageReaction, user: DiscordUser) {
        if (user.bot) {
            return;
        }
        const option = PollDiscord.options.indexOf(messageReaction.emoji.name);
        if (option > -1) {
            await PollDiscord.changeVote(messageReaction.message.id, option, user, true);
        }
    }
    ...
    public async removePoll(poll: Poll) {
        if (poll.messageID) {
            const msg = PollDiscord.getChannel().messages.get(poll.messageID);
            if (msg) {
                await msg.delete();
                poll.messageID = '';
                poll.save();
